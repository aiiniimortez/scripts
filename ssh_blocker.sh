...
# -------------------------
# 1) Check/install prerequisites (quiet & efficient)
# -------------------------
...
# -------------------------
# Option 3: Change SSH Port (enterprise-safe)
# -------------------------
if [[ "$choice" == "3" ]]; then
  echo
  ...
  if ! sudo sshd -t; then
      ...
      sudo cp "${SSHD_MAIN}.bak.$(date +%F_%T)" "$SSHD_MAIN" 2>/dev/null || true
      sudo systemctl restart ssh
      sudo iptables -D INPUT -p tcp --dport "$NEWPORT" -j ACCEPT || true
      exit 1
  fi

  echo "[+] Restarting SSH (socket-aware)..."
